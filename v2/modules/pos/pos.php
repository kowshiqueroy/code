...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'finalize_sale') {
    ...
    $payMethods  = array_filter($payMethods, fn($m) => in_array($m, ['cash','card','transfer']));
...
<div class="pos-layout">
  ...
  <div class="cart-panel">
    ...
    <!-- Discount / VAT with % or $ toggle -->
    <div style="padding:12px 16px;border-bottom:1px solid var(--border)">
      <?php
      $sRaw = dbFetchAll('SELECT `key`,`value` FROM settings');
      $S = []; foreach($sRaw as $r) $S[$r['key']]=$r['value'];
      $discEnabled = ($S['discount_enabled'] ?? '1') == '1';
      $vatEnabled  = ($S['vat_enabled']      ?? '1') == '1';
      $discDefault = $S['discount_default']  ?? 0;
      $vatDefault  = $S['vat_default']       ?? 15;
      $discType    = $S['discount_type']     ?? 'both';
      ?>
      <?php if ($discEnabled): ?>
      <div class="form-row cols-2" style="margin-bottom:8px">
        <div class="form-group" style="margin-bottom:0">
          ...
          <div style="display:flex;gap:4px">
            ...
            <select id="discountType" class="form-control" style="max-width:60px" oninput="Cart.updateTotals()">
              <option value="percent">%</option><option value="amount">$</option>
            </select>
            <?php else: ?>
            <input type="hidden" id="discountType" value="<?= $discType ?>">
            <span class="form-control" style="max-width:50px;text-align:center"><?= $discType==='percent'?'%':'$' ?></span>
            <?php endif ?>
            <input type="number" id="discountPct" class="form-control" value="<?= $discDefault ?>" min="0" step="0.5"
                   oninput="Cart.updateTotals()">
          </div>
        </div>
        <?php if ($vatEnabled): ?>
        <div class="form-group" style="margin-bottom:0">
          ...
          <div style="display:flex;gap:4px">
            <select id="vatType" class="form-control" style="max-width:60px" oninput="Cart.updateTotals()">
              <option value="percent">%</option><option value="amount">$</option>
            </select>
            <input type="number" id="vatRate" class="form-control" value="<?= $vatDefault ?>" min="0" step="0.5"
                   oninput="Cart.updateTotals()">
          </div>
        </div>
        <?php endif ?>
      </div>
      <?php elseif ($vatEnabled): ?>
      <div class="form-group" style="margin-bottom:0">
        <label class="form-label">VAT</label>
        <div style="display:flex;gap:4px">
          <select id="vatType" class="form-control" style="max-width:60px" oninput="Cart.updateTotals()">
    ...
    <form method="POST" style="padding:12px 16px" id="checkoutForm">
      ...
      <div class="form-group">
        <label class="form-label">Payment Methods <small class="text-muted">(select all that apply)</small></label>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <?php foreach(['cash'=>'💵 Cash','card'=>'💳 Card','transfer'=>'🏦 Transfer'] as $val=>$label): ?>
          <label style="display:flex;align-items:center;gap:5px;padding:7px 12px;border-radius:8px;
                        border:1px solid var(--border);cursor:pointer;font-size:.85rem;font-weight:600;
                        transition:border-color .15s" class="pay-opt-wrap">
            <input type="checkbox" name="payment_methods[]" value="<?= $val ?>"
                   <?= $val==='cash'?'checked':'' ?> class="pay-check"
