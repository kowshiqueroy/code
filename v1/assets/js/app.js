...
const Cart = (() => {
  ...
  function updateTotals() {
    ...
    const ptUsed   = parseInt(document.getElementById('pointsUsed')?.value      || 0);
    const ptVal    = ptUsed * 0.01;                      // 1 pt = $0.01

    const discAmt  = sub * (discPct / 100);
    const afterDis = sub - discAmt - ptVal;
    const vatAmt   = afterDis * (vatRate / 100);
    const total    = afterDis + vatAmt;
  ...
  fetch(`index.php?page=pos&action=lookup_customer&phone=${encodeURIComponent(phone)}`)
    ...
    .then(c => {
      if (c.id) {
        ...
        setValue('pointsBalance', c.points);
        setText('pointsBadge',   `${c.points} pts`);
